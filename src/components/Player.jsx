...
const VolumeControl = () => {
    const volume = usePlayerStore(state => state.volume) || 0;  // Ensure volume is never undefined
    const setVolume = usePlayerStore(state => state.setVolume);
    const previousVolumeRef = useRef(volume);  // Remember the previous volume

    const isVolumeSilenced = volume < 0.1;

    const handleClickVolumen = () => {
        if (isVolumeSilenced) {
            setVolume(previousVolumeRef.current);  // Restore previous volume if muted
        } else {
        ...
        <div className="flex justify-center gap-x-2 text-white">
            ...
            <Slider
                ...
                onValueChange={(value) => {
                    ...
                    setVolume(newVolume / 100);  // Update volume based on slider's percentage
                }}
            />
        </div>
    );
